...
export default function EditarAlerta() {
  ...
  const { registro, carregando, erro } = useRegistro<Alerta>(
    alertasApi.mostrar,
    id,
  );
  ...
  const salvar = async (dados: DadosAlerta) => {
    ...
    try {
      await alertasApi.atualizar(id, dados);

      navegar("/alertas", {
        state: { mensagem: "Alerta atualizado com sucesso." },
      });
    } catch (falha) {
  ...
  return (
    <div>
      <PageMeta
        title="Editar alerta | WhatsApp Manager"
        description="Edição de alerta de oferta de passagem"
      />
      <CabecalhoPagina
